...
-- Create a combined function that does both status transition and processing
...
BEGIN
  -- First transition ended auctions
  v_transitioned_count := public.transition_ended_auctions();
  
  -- Then process them for results tables
  v_processed_count := public.process_ended_auctions();
