-- Drop both overloaded versions with exact signatures
DROP FUNCTION IF EXISTS admin_transfer_manual_valuation_to_cars_enhanced(p_manual_valuation_id uuid, p_reserve_price numeric);
DROP FUNCTION IF EXISTS admin_transfer_manual_valuation_to_cars_enhanced(p_valuation_id uuid, p_reserve_price numeric, p_admin_id uuid);

-- Recreate the function with title, finance_amount, and finance document fields
...
BEGIN
  ...
  IF v_valuation.make IS NOT NULL AND v_valuation.model IS NOT NULL AND v_valuation.year IS NOT NULL THEN
    v_generated_title := v_valuation.year || ' ' || UPPER(v_valuation.make) || ' ' || UPPER(v_valuation.model);
  ELSE
    v_generated_title := 'Car Listing';
  END IF;

  -- Insert into cars table with ALL fields including title, finance_amount, and finance documents
  INSERT INTO cars (
    ...
    title, finance_amount, finance_document_url, finance_document_name, finance_document_uploaded_at,
...
COMMENT ON FUNCTION admin_transfer_manual_valuation_to_cars_enhanced IS 'Transfer manual valuation to cars table with title generation, finance amount, and finance document fields';