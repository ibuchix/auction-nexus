-- Create manual-valuations storage bucket if it doesn't exist
INSERT INTO storage.buckets (id, name, public)
VALUES ('manual-valuations', 'manual-valuations', false)
ON CONFLICT (id) DO NOTHING;

-- Create RLS policies for manual-valuations bucket
CREATE POLICY "Admins can access all manual valuation files" 
ON storage.objects 
FOR ALL 
USING (
  bucket_id = 'manual-valuations' 
  AND EXISTS (
...
CREATE POLICY "Sellers can access their own manual valuation files" 
ON storage.objects 
FOR ALL 
USING (
  bucket_id = 'manual-valuations' 
  AND EXISTS (
    SELECT 1 FROM manual_file_uploads mfu
    JOIN manual_valuations mv ON mfu.manual_valuation_id = mv.id
    WHERE mfu.file_path = storage.objects.name
...
-- Create or replace the admin_get_manual_valuations RPC function
...
BEGIN
  ...
  LOOP
    ...
    v_valuations := v_valuations || jsonb_build_object(
      ...
      'address', v_valuation_record.address,
