-- Fix admin_end_auction_immediately to call the correct function
-- Change process_ended_auction(p_car_id) to process_ended_auctions_securely()
-- which exists and will process all ended auctions including the one we just set
...
BEGIN
  ...
  -- Validate that auction is in a state that can be ended
  ...
  -- Call the existing process_ended_auctions_securely function
  -- This will find and process all auctions where end_time <= NOW(), including ours
